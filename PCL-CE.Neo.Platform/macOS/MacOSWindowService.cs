...
public class MacOSWindowService : IWindowService
{
    ...
    public MacOSWindowService(ILogger<MacOSWindowService> logger)
    {
        ...
        try
        {
            _logger.LogDebug("MacOSWindowService initializing");
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Error during MacOSWindowService initialization");
        }
    }
    ...
    public double GetSystemDpi()
    {
        try
        {
            ...
            if (process != null)
            {
                ...
                {
                    var dpiLine = output.Split('\n', StringSplitOptions.RemoveEmptyEntries)
                        .FirstOrDefault(l => l.Contains("Resolution", StringComparison.OrdinalIgnoreCase));
                    _logger.LogDebug("Display info: {Info}", dpiLine ?? "unknown");
                }
            }
        }
        catch (Exception ex)
