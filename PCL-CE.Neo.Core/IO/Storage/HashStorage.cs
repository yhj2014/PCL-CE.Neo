...
{
    private readonly ILogger<HashStorage>? _logger = null;

    public HashStorage(string folder, IHashProvider hashProvider, ILogger<HashStorage>? logger, bool compressObjects = false, bool correctMisplacedFile = true, int prefixLength = 2)
        : this(folder, hashProvider, compressObjects, correctMisplacedFile, prefixLength)
    {
        _logger = logger;
    }
    ...
    public async Task<string?> PutAsync(Stream input, string? hash = null)
    {
        ...
        if (hash is not null && hash.Length != hashProvider.Length)
            throw new ArgumentException("Provided hash is not correct", nameof(hash));
        ...
        if (correctMisplacedFile && _CorrectMisplacedFile(fileHash))
            _logger?.LogInformation("[HashStorage] Move misplaced file into correct folder");
    ...
    public Task<bool> DeleteAsync(string hash)
    {
        ...
        {
            _logger?.LogError(ex, "[HashStorage] Unexpected directory not found {FilePath}", filePath);
            return Task.FromResult(false);
        }
        catch (IOException ex)
        {
            _logger?.LogError(ex, "[HashStorage] Failed to delete file {FilePath}", filePath);
            return Task.FromResult(false);
        }
        catch (UnauthorizedAccessException ex)
        {
            _logger?.LogError(ex, "[HashStorage] Access denied when deleting file {FilePath}", filePath);
            return Task.FromResult(false);
        }
    ...
    public Stream? Get(string hash)
    {
        ...
        if (correctMisplacedFile && _CorrectMisplacedFile(hash))
            _logger?.LogInformation("[HashStorage] Move misplaced file into correct folder: {Hash}", hash);
