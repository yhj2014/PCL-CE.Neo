...
{
    private const string ModuleName = "HashStorage";
    ...
    public async Task<string?> PutAsync(Stream input, string? hash = null)
    {
        ...
        if (correctMisplacedFile && _CorrectMisplacedFile(fileHash)) 
            LogWrapper.Info(ModuleName, "Move misplaced file into correct folder");
    ...
    public Task<bool> DeleteAsync(string hash)
    {
        ...
        {
            LogWrapper.Error(ex, ModuleName, $"Unexpected directory not found {filePath}");
            return Task.FromResult(false);
        }
        catch (IOException ex)
        {
            LogWrapper.Error(ex, ModuleName, $"Failed to delete file {filePath}");
            return Task.FromResult(false);
        }
        catch (UnauthorizedAccessException ex)
        {
            LogWrapper.Error(ex, ModuleName, $"Access denied when deleting file {filePath}");
            return Task.FromResult(false);
        }
    ...
    public Stream? Get(string hash)
    {
        ...
        if (correctMisplacedFile && _CorrectMisplacedFile(hash))
            LogWrapper.Info(ModuleName, $"Move misplaced file into correct folder: {hash}");
