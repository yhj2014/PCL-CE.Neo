...
public static class PEHeaderReader
{
    ...
    public static PEStruct ReadPEHeader(string filePath)
    {
        ...
        {
            result.IsValid = false;
            result.ErrorMessage = $"访问权限异常: {ex.Message}";
        }
        catch (Exception ex)
        {
            result.IsValid = false;
    ...
    /// 判断机器架构是否为 64 位
    /// </summary>
    /// <param name="machine">机器类型</param>
    /// <returns>是否为 64 位</returns>
    public static bool IsMachine64Bit(MachineType machine)
    {
        return machine == MachineType.IA64 
            || machine == MachineType.ARM64 
            || machine == MachineType.AMD64;
    }

    /// <summary>
    /// 判断 PE 结构的机器架构是否为 64 位（便捷方法）
    /// </summary>
    /// <param name="peStruct">PE 结构</param>
    /// <returns>是否为 64 位</returns>
    public static bool IsMachine64Bit(PEStruct peStruct)
    {
        return IsMachine64Bit(peStruct.Machine);
    }
