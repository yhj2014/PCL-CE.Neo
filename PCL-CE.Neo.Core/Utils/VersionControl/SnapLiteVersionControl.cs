...
public class SnapLiteVersionControl : IVersionControl, IDisposable
{
    ...
    public SnapLiteVersionControl(string rootPath, ILogger<SnapLiteVersionControl> logger)
    {
        _logger = logger;
        try
        {
    ...
    public async Task<string> CreateNewVersion(string? name = null, string? desc = null)
    {
        try
        {
            ...
            _logger.LogInformation("[SnapLite] 记录已压入数据库，开始存储文件");
            ...
            _logger.LogInformation("[SnapLite] 数据库记录更新完成");
            return nodeId;
        }
        catch (Exception e)
        {
            ...
            .ForEachAsync(deleteFile => Task.Run(() =>
            {
                try
                {
                    ...
                    else if (deleteFile.ObjectType == ObjectType.Directory)
                    {
                        ...
                        {
                            _logger.LogWarning(e, "目录 {Directory} 可能已受到外部修改，存在不属于此版本的内容且删除操作失败", curDir);
                        }
                    }
                }
                catch (Exception e)
