...
public sealed class AesGcmProvider : IEncryptionProvider
{
    ...
    public byte[] Encrypt(ReadOnlySpan<byte> data, ReadOnlySpan<byte> key)
    {
        ...
        using var aes = new AesGcm(outputKey);
        ...
        ciphertext.AsSpan().CopyTo(resultSpan[(SaltSize + NonceSize + TagSize)..]);
    ...
    private static readonly byte[] _Info = "PCL_CE.Neo.Core.Utils.Encryption.AesGcm"u8.ToArray();
