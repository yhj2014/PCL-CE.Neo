...
public sealed class ChaCha20Poly1305Provider : IEncryptionProvider
{
    ...
    private static readonly byte[] Info = "PCL_CE.Neo.Core.Utils.Encryption.ChaCha20"u8.ToArray();

    private static void DeriveKey(ReadOnlySpan<byte> ikm, ReadOnlySpan<byte> salt, Span<byte> outputKey)
    {
        HKDF.DeriveKey(
