...
public class BsDiff : IBinaryDiff
{
    ...
    public async Task<byte[]> ApplyAsync(byte[] originData, byte[] diffData)
    {
        return await Task.Run(() =>
        {
            ...
            using var ctrlStream = new BZip2Stream(new MemoryStream(ctrlContent), CompressionMode.Decompress);
            using var ctrlReader = new BinaryReader(ctrlStream);

            using var diffStream = new BZip2Stream(new MemoryStream(diffContent), CompressionMode.Decompress);
            using var diffReader = new BinaryReader(diffStream);

            using var extraStream = new BZip2Stream(new MemoryStream(extraContent), CompressionMode.Decompress);
    ...
    {
        throw new NotSupportedException("BsDiff Make is not implemented. Use a third-party library for creating patches.");
    }
