...
public class BsDiff : IBinaryDiff
{
    ...
    private readonly ILogger<BsDiff> _logger;

    public BsDiff(ILogger<BsDiff> logger)
    {
        _logger = logger;
    }
    ...
    public async Task<byte[]> ApplyAsync(byte[] originData, byte[] diffData)
    {
        try
        {
            if (diffData.Length < HeaderSize)
                throw new ArgumentException("Diff file size is less than the header size");
            if (BitConverter.ToInt64(diffData, HeaderVersionIndex) != HeaderVersion)
                throw new ArgumentException("Diff file version is wrong");
            ...
            if (ctrlLen < 0 || diffLen < 0 || extraLen < 0)
                throw new ArgumentException("Block size is negative");
            if (newLen < 0)
                throw new ArgumentException("Final file size info is negative");
            if (HeaderSize + ctrlLen + diffLen + extraLen > diffData.Length)
                throw new ArgumentException("Diff file size info is not correct");

            _logger.LogDebug(
                "Got diff-data-len = {DiffDataLength}, ctrllen = {CtrlLen}, difflen = {DiffLen}, extralen = {ExtraLen}, totallen = {NewLen}",
                diffData.Length, ctrlLen, diffLen, extraLen, newLen);
            ...
            while (newDataPos < newLen)
            {
                ...
                _logger.LogTrace(
                    "Round add-range = {AddRange}, copy-range = {CopyRange}, seek-pos = {SeekPos}",
                    addRange, copyRange, seekPos);
            ...
            _logger.LogDebug("BsDiff apply completed successfully");
            return ret;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to apply BsDiff");
            throw;
        }
    }
    ...
    public Task<byte[]> MakeAsync(byte[] originData, byte[] newData)
    {
        try
        {
            _logger.LogWarning("BsDiff MakeAsync is not implemented");
            throw new NotImplementedException("BsDiff MakeAsync is not implemented");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "BsDiff MakeAsync failed");
            throw;
        }
    }
