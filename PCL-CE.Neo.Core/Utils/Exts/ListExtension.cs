...
{
    public static T? MaxOrDefault<C>(this IEnumerable<T> source, Func<T, C> selector) where C : IComparable<C>
    {
    ...
    public static T? MinOrDefault<C>(this IEnumerable<T> source, Func<T, C> selector) where C : IComparable<C>
    {
        using var enumerator = source.GetEnumerator();
        if (!enumerator.MoveNext()) return default;
