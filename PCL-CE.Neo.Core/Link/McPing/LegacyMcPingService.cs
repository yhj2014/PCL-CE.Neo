...
public class LegacyMcPingService : IMcPingService
{
    ...
    public LegacyMcPingService(IPEndPoint endpoint, ILogger<LegacyMcPingService>? logger = null, int timeout = DefaultTimeout)
    {
        _endpoint = endpoint;
        _host = _endpoint.Address.ToString();
        _timeout = timeout;
        _logger = logger ?? Microsoft.Extensions.Logging.Abstractions.NullLogger<LegacyMcPingService>.Instance;
    }

    public LegacyMcPingService(string ip, ILogger<LegacyMcPingService>? logger = null, int port = 25565, int timeout = DefaultTimeout)
    {
    ...
    public async Task<McPingResult?> PingAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogError(e, "Failed to read response from {Endpoint}", _endpoint);
            return null;
        }
        ...
        {
            _logger.LogError(e, "Unable to parse response from {Endpoint}", _endpoint);
            return null;
        }
    }
