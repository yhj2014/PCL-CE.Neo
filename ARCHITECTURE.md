...
本文档描述 PCL Community Edition (PCL CE) 跨平台重构的架构设计。

### 1.1 目标
将 PCL CE 从仅支持 Windows 的 WPF 应用，重构为基于 .NET 10 和 Uno Platform 的跨平台应用，支持 Windows、macOS 和 Linux。
...
│   └── Platforms/                     # 各平台入口
│
└── Plain Craft Launcher 2/           # 【原 WPF 版本，保留】
```
...
PCL.App (跨平台)
    ...
    ├── PCL.Platform.Windows (或 MacOS/Linux)
    │   └── PCL.Core.Abstractions
    └── PCL.Core
        └── PCL.Core.Abstractions
```
